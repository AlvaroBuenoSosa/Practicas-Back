...
public class ClienteServiceImpl implements ClienteService{
	...
	@Override
    public ClienteReportDto getClienteReport(Integer id) {
        Cliente cliente = clientesRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Cliente no encontrado"));

        ClienteReportDto dto = new ClienteReportDto();
        dto.setClienteId(cliente.getId());
        dto.setNombre(cliente.getNombre());

        // Initialize the list for accounts
        List<ClienteReportDto.CuentaDto> cuentaDtos = new ArrayList<>();
        
        // Convert each account to DTO
        for (Cuenta cuenta : cliente.getCuentas()) {
            ClienteReportDto.CuentaDto cuentaDto = new ClienteReportDto.CuentaDto();
            ...
            List<ClienteReportDto.MovimientoDto> movimientoDtos = new ArrayList<>();
            
            // Convert each movement to DTO
            for (Movimiento movimiento : cuenta.getMovimiento()) {
                ClienteReportDto.MovimientoDto movimientoDto = new ClienteReportDto.MovimientoDto();
